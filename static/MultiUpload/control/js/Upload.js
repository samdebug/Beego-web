		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '			<div class="upload_choose" style="display:none">';
		            ...
		            html += '				<div id="status_info" class="info">（最多可选择八张）选中0张文件</div>';
		            html += '				<div class="btns">';
					...
					/*html += '				<div class="upload_preview_span">';
					html += '					<i class="lnr lnr-upload"></i>';
					html += '					<span>上传多张照片</span>';
					html += '				</div>';*/
				    html += '			</div>';
					html += '		</div>';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">选择文件</div>';
		            html += '					<div class="upload_btn">开始上传</div>';
				    ...
				    html += '						<div class="uploadImg" style="width:100px;height">';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				//$("#status_info").html("（最多可选择八张）" + "选中"+num+"张文件，共"+size+"。");
				$("#status_info").html("（最多可选择八张）" + "选中"+num+"张文件");
			};
			 ...
			 * 返回: 预览的html
			 */
			this.funDisposePreviewHtml = function(file, e,realwidth,realheight,imgSrc){
				//计算居中
				function AutoResizeImage(realwidth,realheight,maxWidth,maxHeight,imgSrc){
			        var img = new Image();
			        img.src = imgSrc ;
			        var hRatio;
			        var wRatio;
			        ...
			        return {"handleheight":(Math.round(h) + "px"),"handlewidth":(Math.round(w) + "px")};

			        if(h < maxHeight) { // 纵向有空余空间
			            console.log(Math.round((maxHeight - h) / 2) + "px");
			        }
			        if(w < maxWidth) { // 横向有空余空间
			        	 console.log(Math.round((maxWidth - w) / 2) + "px");
			        }
			     }
			    var Imgdata = AutoResizeImage(realwidth,realheight,100,122,imgSrc);
				var handlewidth = Imgdata.handlewidth;
				var handleheight = Imgdata.handleheight;
				...
				var delHtml = "";
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del fa fa-times" data-index="'+file.index+'" title="删除"></span>';
				}
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		<div class="uploadImg" style="width:100px;height:122px;display: flex;align-items: center;justify-content: center;">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:' + handlewidth + ';height:' + handleheight+';" />';                                                                 
					html += '		</div>';
					html += '	</a>';
					...
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox">';
					html += '		<div class="uploadImg" style="width:100px;height:122px;display: flex;align-items: center;justify-content: center;">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + fileImgSrc + '" style="width:' + handlewidth + ';height:' + handleheight+';" />';                                                                 
					html += '		</div>';
					html += '	</a>';
					//html += '	<p id="uploadProgress_'+file.index+'" class="file_progress"></p>';
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure"></p>';
							...
							if (file) {
								...
								reader.onload = function(e) {
								    ...
								    image.onload=function(){  
								        ...
								        html += self.funDisposePreviewHtml(file, e, realwidth, realheight,data);
										i++;
